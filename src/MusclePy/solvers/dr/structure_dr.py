...
class Structure_DR(FEM_Structure):
    ...
    def __init__(self, nodes: FEM_Nodes, elements: FEM_Elements):
        ...
        # Initialize DR-specific attributes using utility functions
        ...
        # Compute local geometric stiffness matrices and convert to global matrix
        local_geo_matrices = compute_local_geometric_stiffness_matrices(self.elements, self.nodes)
        self.global_geometric_stiffness_matrix = local_to_global_stiffness_matrix(
            local_geo_matrices, self.elements, self.nodes.count
        )
