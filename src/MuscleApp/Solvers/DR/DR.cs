...
namespace MuscleApp.Solvers
{
    ...
    {
        /// <summary>
        /// Solve the dynamic relaxation method for a structure with incremental loads and prestress (free length changes).
        /// </summary>
        ...
        public static Truss? Solve(Truss truss, IEnumerable<PointLoad> pointLoads, IEnumerable<Prestress> prestress, CoreConfigDR config)
        {
            ...
            try{
                coreResult = MuscleCore.Solvers.DynamicRelaxation.Solve(TrussEncoder.ToCore(truss), loadsIncrement, freeLengthVariation, config);
            }
            catch (Exception)
            {
