...
plt.xlabel('Précontrainte [kN]')
plt.ylabel('Omega [Rad/sec]')
plt.title('Evolution des 5 premières fréquences propres en fonction de la précontrainte')
plt.legend()
...
ax.set_title('Variation des 5 premières fréquences propres en fonction de l\'augmentation de la précontrainte')
...
for i in range(Str.NodesCount):
    # ax.text(CoordX[i],CoordY[i],CoordZ[i], 'Node '+str(NodeNum[i]) +'-'+ str((round(CoordX[i], 2),round(CoordY[i], 2),round(CoordZ[i], 2))), color="black", fontsize=10,fontweight='bold')
