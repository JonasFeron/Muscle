...
class NonlinearDisplacementMethod:
    """Core implementation of the nonlinear displacement method."""

    @staticmethod
    def incremental_newton_raphson_procedure(structure: FEM_Structure, loads_increment: np.ndarray, n_steps: int) -> FEM_Structure:
        ...
        Returns:
            Structure_Linear_DM in deformed state
        """
        # Note: the _equivalent_prestress_loads method used in the LinearDisplacementMethod does not work with the nonlinear DM.
        ...
        loads_incr = np.zeros((3 * initial_state.nodes.count,)) # a fraction of the total load increment
        displacements_incr = np.zeros((3 * initial_state.nodes.count,))
        resisting_forces_incr = np.zeros((3 * initial_state.nodes.count,))
        tensions_incr = np.zeros((initial_state.elements.count,))
        reactions_incr = np.zeros((initial_state.nodes.fixations_count,))
