...
def main(json_data):
    ...
    json_result = json.dumps(result, cls=r.SharedSolverResultEncoder, ensure_ascii=False) #Results are saved as dictionnary JSON. # , indent="\t"
    return json_result
...
def core(data):
    ...
    if isinstance(data, d.SharedData):#check that data is a SharedData object !
        ...
        Struct.MainNONLinearDisplacementMethod(data) #do some calculations
