...
namespace Muscle.GHModel
{
    public class GH_Support : GH_GeometricGoo<Support>, IGH_PreviewData
    {
        ...
        public void DrawViewportMeshes(GH_PreviewMeshArgs args) // draw a cone for each fixed translation 
        {
            ...
            if (!Value.isZFree) { args.Pipeline.DrawCone(new Cone(new Plane(Value.Point, new Vector3d(0.0, 0.0, -1.0)), height, radius), Color.Blue); }
        }

        public void DrawViewportWires(GH_PreviewWireArgs args)
        {
            args.Pipeline.DrawPoint(Value.Point, Color.Blue);
